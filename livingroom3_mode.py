...
def enter():
    ...
    # 새로운 객체 생성
    background = Background('livingroom3.png', 600, 400)  # 홀 이미지 생성
    girl = Girl()  # 소녀 객체 생성
    ...
    for i, transition_box in enumerate(transition_boxes):
        if check_for_transition(girl, transition_box):
            if i == 0:
                ...
                print('ending ')
                import happyend_mode
                #happyend_mode.set_girl_position(800, 400)  # 시작 위치 설정
                game_framework.change_mode(happyend_mode)
                pass
